### ✨ PLANTILLA README – GitHub ✨
#####  Nombre del Proyecto: 
Manejo de Archivos de Texto y Validaciones en C#

### Descripción

Este proyecto consiste en una aplicación de escritorio desarrollada en C# que permite registrar los datos de los estudiantes y sus calificaciones.
El sistema valida que los campos estén completos, calcula el promedio de las notas y muestra mensajes indicando si el estudiante aprobó o reprobó.

La aplicación fue realizada como práctica de aula, siguiendo las indicaciones explicadas en clase.

### Tecnologías Utilizadas

- Lenguaje: C#

- Framework: .NET (Windows Forms)

- Entorno de Desarrollo (IDE): Visual Studio

- Tipo de Aplicación: Escritorio

###  Funcionalidades

##### La aplicación cuenta con las siguientes funcionalidades:

- Validar que los campos no estén vacíos:

![imagen alt](https://github.com/Jeimy-Nathaly/Calificacion-estudiantil/blob/main/one.png?raw=true)


- Registro correcto de calificaciones y Cálculo del promedio:
![imagen alt](https://github.com/Jeimy-Nathaly/Calificacion-estudiantil/blob/main/two.png?raw=true)

- Mensaje según resultado (por completivo):
![imagen alt](https://github.com/Jeimy-Nathaly/Calificacion-estudiantil/blob/main/three.png?raw=true)

- Mensaje según resultado (por extraodinario):
![imagen alt](https://github.com/Jeimy-Nathaly/Calificacion-estudiantil/blob/main/four.png?raw=true)

- Mensaje según resultado (reprobado):
![imagen alt](https://github.com/Jeimy-Nathaly/Calificacion-estudiantil/blob/main/six.png?raw=true)


### Contexto Académico

- Nivel: Secundaria Técnico Profesional
- Modulo Formativo: Desarrollo de Aplicaciones y Sistemas de Información
- Curso: 5to.D1
- Año Escolar: 2025-2026



### Autores

##### ✨Jeimy Nathaly Concepción Ramírez. #7✨







